...
Analyze this location, predict typical forecast/climate, aesthetic vibe, and curate a minimal capsule wardrobe & packing checklist.
...
Guidelines:
1. "weather_summary": Realistic climate prediction for ${location}.
2. "auto_vibe": Aesthetic fashion style for ${location}.
3. "capsule_wardrobe": 5-7 minimalist, high-quality versatile pieces.
4. "outfit_combinations": 3-5 daily combinations using the capsule items.
5. "essentials": Categorized packing items tailored specifically to the climate and travel requirements.
...
`;
  ...
  try {
    ...
    if (!response.ok) {
      console.warn('Gemini request status non-OK:', response.status);
      return generateFallbackPackingPlan(location, durationDays);
    }

    const data = await response.json();
    const rawText = data?.candidates?.[0]?.content?.parts?.[0]?.text;
    if (!rawText) return generateFallbackPackingPlan(location, durationDays);

    const cleanedText = rawText.replace(/```json/g, '').replace(/```/g, '').trim();
    const parsed = JSON.parse(cleanedText);

    // Format essentials into flat array with category labels
    ...
    return {
      ...
      city_info: {
        weather_summary: parsed.city_info?.weather_summary || '20°C • Temperate & Mild',
        auto_vibe: parsed.city_info?.auto_vibe || 'Minimalist Chic',
        local_highlights: parsed.city_info?.local_highlights || [
          'Stay hydrated with a refillable water bottle during long walking tours.',
          'Keep digital screenshots of your flight & lodging bookings offline.',
          'Pack versatile neutral layers to easily adjust to temperature shifts.'
        ],
      },
